...
function saveState(): void {
    vscode.setState({ commitMessage, commitMode, stagedCollapsed, unstagedCollapsed });
}
...
function renderFilesList(): void {
    ...
    if (!statusData || (statusData.staged.length === 0 && statusData.unstaged.length === 0)) {
    ...
    if (statusData.unstaged.length > 0) {
