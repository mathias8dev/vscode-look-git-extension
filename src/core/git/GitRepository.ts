import type { GitCommit, GitFileChange, GraphCommit } from './domain/GitCommit';
import type { GitStatus, StashEntry, BranchInfo, TagInfo } from './domain/GitStatus';
import type { WorktreeInfo, SubmoduleInfo } from './domain/GitWorktree';
...
/** Injected into use-cases. AbortSignal lives in exec() only — not threaded through every method. */
export interface GitRepository {
    ...
    stashList(signal?: AbortSignal): Promise<readonly StashEntry[]>;
    ...
    getGraphLog(maxCount: number, branches?: readonly string[], pathFilter?: string, filters?: GraphLogFilters, signal?: AbortSignal): Promise<readonly GraphCommit[]>;
    ...
    // Branches & tags
    getAllBranches(signal?: AbortSignal): Promise<readonly BranchInfo[]>;
    getAllTags(signal?: AbortSignal): Promise<readonly TagInfo[]>;
    ...
    // Worktrees
    listWorktrees(signal?: AbortSignal): Promise<readonly WorktreeInfo[]>;
    ...
    // Submodules
    getSubmoduleStatus(signal?: AbortSignal): Promise<readonly SubmoduleInfo[]>;
