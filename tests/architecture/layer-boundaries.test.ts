...
describe('architecture layer boundaries', () => {
    ...
    it('uses kebab-case filenames for source and test files', () => {
        const violations = [...listSourceFiles(SRC_ROOT), ...listSourceFiles(TESTS_ROOT)]
            .filter((filePath) => !isKebabCaseSourceFileName(filePath))
...
function isKebabCaseSourceFileName(filePath: string): boolean {
    const parts = path.basename(filePath).split('.').slice(0, -1);
